  ...
  fichiers
    ...
    if (clientExistant) {
      ...
      const nouveauDocument = new Document({
        ...
        sexe,
        typeDocument: 'voyage',
        pays: Array.isArray(pays) ? pays : [pays],  // S'assurer que c'est un tableau
        raison,
        autreRaison: raison === 'autres' ? autreRaison : undefined,
        dateDebut: new Date(dateDebut),
        dateFin: new Date(dateFin),
    ...
    console.log(`📊 Taille totale: ${(documentSauve.getTailleTotale() / 1024 / 1024).toFixed(2)}MB`);
    ...
    try {
      ...
      // ✅ NOUVEAU : WhatsApp client (asynchrone, non bloquant)
      whatsappService.sendAutoNotification(
        ...
        console.warn('⚠️ Erreur WhatsApp (non bloquant):', err.message);
      });
      
    } catch (notificationError) {
      console.warn('⚠️ Erreur notifications:', notificationError.message);
...
router.post('/submit-transfert', upload.any(), async (req, res) => {
  ...
  fichiers
    ...
    console.log('✅ Document TRANSFERT créé dans MongoDB:', reference);

    // ✅ ENVOYER NOTIFICATIONS (EMAIL + WHATSAPP)
    try {
      // Email admin - ENVOYER LE NOUVEAU SOUS-DOSSIER
      await sendNewSubmissionNotification(nouveauDocument, 'transfert', sousDossier);
    ...
    console.error('❌ Erreur soumission transfert:', error);
...
router.get('/formulaire-limite.pdf', async (req, res) => {
  console.log('📍📍📍 ROUTE /formulaire-limite.pdf APPELÉE 📍📍📍');
  try {
    const formulaire = await Formulaire.findOne({ nom: 'formulaire-limite' });
    console.log('📄 Formulaire trouvé:', formulaire ? 'OUI' : 'NON');

    if (!formulaire) {
      console.log('❌ Formulaire non trouvé dans la base');
  ...
  try {
    ...
    if (statut === 'rejeté' && motifRejet) {
      try {
        ...
        console.log('📧 Résultat envoi notifications:', emailResult);
        
        if (!emailResult.success) {
          console.warn('⚠️ Problème lors de l\'envoi des emails de rejet');
        }
      } catch (emailError) {
...
router.post('/admin/formulaire-client', auth, uploadFormulaire.single('formulaire'), async (req, res) => {
  try {
    ...
    console.log(`✅ Formulaire client sauvegardé dans MongoDB: ${formulaire.nomOriginal}`);
...
router.get('/admin/download/:reference/:sousDossierId/:fileId', auth, async (req, res) => {
  try {
    ...
    console.log(`✅ Fichier téléchargé: ${fichier.nomOriginal} (${(fichier.taille / 1024).toFixed(2)}KB)`);

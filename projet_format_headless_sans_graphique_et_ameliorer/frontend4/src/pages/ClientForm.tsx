...
// Données initiales pour chaque onglet
const initialVoyageData = {
  nom: '',
  prenom: '',
  email: '',
  telephone: '',
...
export default function ClientForm() {
  ...
  const [voyageData, setVoyageData] = useState(initialVoyageData);
  const [transfertData, setTransfertData] = useState(initialTransfertData);
  ...
  // Utiliser les données de l'onglet actif
  const currentFormData = activeTab === 'voyage' ? voyageData : transfertData;
  ...
  const handleTabChange = (tab: string) => {
    // Réinitialiser les erreurs et les fichiers lorsqu'on change d'onglet
    setErrors({});
    setFiles([]);
    setActiveTab(tab);
  };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (activeTab === 'voyage') {
      setVoyageData(prev => ({ ...prev, [name]: value }));
    } else {
  ...
  const validateForm = () => {
    ...
    if (!currentFormData.nom.trim()) newErrors.nom = 'Le nom est requis';
    if (!currentFormData.prenom.trim()) newErrors.prenom = 'Le prénom est requis';
    
    // Validation email avec vérification du @
    if (!currentFormData.email.trim()) newErrors.email = 'L\'email est requis';
    else if (!currentFormData.email.includes('@')) newErrors.email = 'L\'email doit contenir le caractère "@"';
    
    // Validation téléphone (chiffres et + uniquement)
    if (!currentFormData.telephone.trim()) newErrors.telephone = 'Le téléphone est requis';
    else if (!/^[0-9+]+$/.test(currentFormData.telephone.replace(/\s/g, ''))) {
      newErrors.telephone = 'Le téléphone ne doit contenir que des chiffres et le signe +';
    }
    
    if (!currentFormData.profession.trim()) newErrors.profession = 'La profession est requise';
    if (!currentFormData.sexe) newErrors.sexe = 'Le sexe est requis';
    
    if (activeTab === 'voyage') {
      if (!voyageData.dateDebut) newErrors.dateDebut = 'La date de début est requise';
      if (!voyageData.dateFin) newErrors.dateFin = 'La date de fin est requise';
    }
    
    if (activeTab === 'voyage') {
      if (!voyageData.pays || voyageData.pays.length === 0) newErrors.pays = 'Au moins un pays est requis';
      if (!voyageData.raison) newErrors.raison = 'La raison du voyage est requise';
      if (voyageData.raison === 'autres' && !voyageData.autreRaison.trim()) {
        newErrors.autreRaison = 'Veuillez préciser la raison';
      }
    } else {
      if (!transfertData.typeTransfert) newErrors.typeTransfert = 'Le type de transfert est requis';
    }
  ...
  const sendWhatsAppMessage = async (telephone: string, type: string) => {
    try {
      const message = type === 'voyage' 
        ? `Bonjour ${voyageData.prenom} ${voyageData.nom}, nous avons bien reçu vos documents de voyage. Notre équipe les examine actuellement. Nous vous contacterons bientôt.`
        : `Bonjour ${transfertData.prenom} ${transfertData.nom}, nous avons bien reçu votre dossier de transfert (${transfertData.typeTransfert}). Notre équipe l'examine actuellement. Nous vous contacterons bientôt.`;
      ...
      const response = await fetch('http://172.237.112.125:5000/api/whatsapp/send', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          message: message,
          clientName: type === 'voyage' ? `${voyageData.prenom} ${voyageData.nom}` : `${transfertData.prenom} ${transfertData.nom}`,
          type: type
        }),
      });
      ...
      if (result.success) {
        ...
        console.warn('WhatsApp échoué:', result.error);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (activeTab === 'voyage') {
        formDataToSend.append('nom', voyageData.nom);
        formDataToSend.append('prenom', voyageData.prenom);
        formDataToSend.append('email', voyageData.email);
        formDataToSend.append('telephone', voyageData.telephone);
        formDataToSend.append('profession', voyageData.profession);
        formDataToSend.append('sexe', voyageData.sexe);
        formDataToSend.append('dateDebut', voyageData.dateDebut);
        formDataToSend.append('dateFin', voyageData.dateFin);
        voyageData.pays.forEach((pays: string) => {
          formDataToSend.append('pays', pays);
        });
        formDataToSend.append('raison', voyageData.raison);
        if (voyageData.autreRaison) {
          formDataToSend.append('autreRaison', voyageData.autreRaison);
        }
      } else {
        formDataToSend.append('nom', transfertData.nom);
        formDataToSend.append('prenom', transfertData.prenom);
        formDataToSend.append('email', transfertData.email);
        formDataToSend.append('telephone', transfertData.telephone);
        formDataToSend.append('profession', transfertData.profession);
        formDataToSend.append('sexe', transfertData.sexe);
        formDataToSend.append('typeTransfert', transfertData.typeTransfert);
        if (transfertData.dateDebut) formDataToSend.append('dateDebut', transfertData.dateDebut);
        if (transfertData.dateFin) formDataToSend.append('dateFin', transfertData.dateFin);
        if (transfertData.entreprise) {
          formDataToSend.append('entreprise', transfertData.entreprise);
        }
      }
      ...
      if (activeTab === 'voyage') {
        console.log('Nom:', voyageData.nom);
        console.log('Prénom:', voyageData.prenom);
        console.log('Email:', voyageData.email);
        console.log('Téléphone:', voyageData.telephone);
        console.log('Profession:', voyageData.profession);
        console.log('Sexe:', voyageData.sexe);
        console.log('Pays:', voyageData.pays.join(', '));
        console.log('Raison:', voyageData.raison);
        console.log('Date début:', voyageData.dateDebut);
        console.log('Date fin:', voyageData.dateFin);
      } else {
        console.log('Nom:', transfertData.nom);
        console.log('Prénom:', transfertData.prenom);
        console.log('Email:', transfertData.email);
        console.log('Téléphone:', transfertData.telephone);
        console.log('Profession:', transfertData.profession);
        console.log('Sexe:', transfertData.sexe);
        console.log('Type transfert:', transfertData.typeTransfert);
        console.log('Entreprise:', transfertData.entreprise);
      }
      ...
      await sendWhatsAppMessage(
        activeTab === 'voyage' ? voyageData.telephone : transfertData.telephone, 
        activeTab
      );
      setShowSuccess(true);
  ...
  const resetForm = () => {
    ...
    setVoyageData(initialVoyageData);
    setTransfertData(initialTransfertData);
    setFiles([]);
    setErrors({});
    setShowSuccess(false);
  };

  // Fonction pour gérer les changements spécifiques aux pays (pour l'onglet voyage)
  const handlePaysChange = (countryName: string, checked: boolean) => {
    setVoyageData(prev => ({
      ...prev,
      pays: checked 
        ? [...prev.pays, countryName]
        : prev.pays.filter(p => p !== countryName)
    }));
  ...
  // Fonction pour gérer le téléphone (identique pour les deux onglets)
  const handleTelephoneChange = (value: string) => {
    const filteredValue = value.replace(/[^0-9+]/g, '');
    
    if (activeTab === 'voyage') {
      setVoyageData(prev => ({ ...prev, telephone: filteredValue }));
    } else {
      setTransfertData(prev => ({ ...prev, telephone: filteredValue }));
    }
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl overflow-hidden">
          <div className="flex border-b border-gray-200">
            <button
              onClick={() => handleTabChange('voyage')}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex space-x-6 pt-3">
                    <label className="flex items-center cursor-pointer">
                      <input
                        ...
                        name="sexe"
                        value="H"
                        checked={currentFormData.sexe === 'H'}
                        onChange={handleInputChange}
                    ...
                    <label className="flex items-center cursor-pointer">
                      <input
                        ...
                        name="sexe"
                        value="F"
                        checked={currentFormData.sexe === 'F'}
                        onChange={handleInputChange}
                        className="w-5 h-5 text-blue-600 focus:ring-blue-500"
                      />
              ...
              {activeTab === 'transfert' && (
                <div>
                  ...
                  <input
                    type="text"
                    name="entreprise"
                    value={transfertData.entreprise}
                    onChange={handleInputChange}
                    className="w-full px-4 py-3 rounded-xl border-2 border-blue-100 focus:border-blue-500 focus:outline-none"
                      ...
                      >
                        <div className="flex justify-between items-center">
                          <span className="text-gray-700">
                            {voyageData.pays.length === 0 
                              ? 'Sélectionnez un ou des pays à visiter' 
                              : `${voyageData.pays.length} pays sélectionné(s)`
                            }
                          </span>
                          <svg 
                              ...
                              {countries.map(country => (
                                <label key={country.name} className="flex items-center justify-between p-2 hover:bg-blue-50 rounded-lg cursor-pointer">
                                  ...
                                  <input
                                    type="checkbox"
                                    value={country.name}
                                    checked={voyageData.pays.includes(country.name)}
                                    onChange={(e) => handlePaysChange(country.name, e.target.checked)}
                          ...
                          {voyageData.pays.map(paysName => {
                            ...
                            return (
                              <span 
                                ...
                                onClick={() => {
                                  handlePaysChange(paysName, false);
                                }}
                              >
              ...
              {activeTab === 'transfert' && (
                <div>
                  ...
                  <select
                    name="typeTransfert"
                    value={transfertData.typeTransfert}
                    onChange={handleInputChange}
                    className={`w-full px-4 py-3 rounded-xl border-2 ${

...
# Test MongoDB avec votre IP
log "Test connexion MongoDB..."
if node -e "require('mongodb').MongoClient.connect('mongodb://172.237.112.125:27017', {serverSelectionTimeoutMS: 3000}).then(client => { console.log('MongoDB OK'); client.close(); process.exit(0); }).catch(err => { console.log('MongoDB erreur:', err.message); process.exit(1); });" 2>/dev/null; then
    log "MongoDB pret"
else
...
log "=== DEMARRAGE APPLICATION EN ARRIERE-PLAN ==="
log "Adresse: http://172.237.112.125:5000"
log "Lancement: node server.js en arriere-plan"

# Demarrer en arriere-plan avec nohup
nohup node server.js > server-app.log 2>&1 &

# Sauvegarder le PID
echo $! > server.pid

log "Serveur demarre avec PID: $(cat server.pid)"
log "Logs application dans: server-app.log"
log "Vous pouvez maintenant vous deconnecter"

# Afficher les dernieres logs
sleep 2
log "=== DERNIERES LOGS APPLICATION ==="
tail -5 server-app.log | tee -a $LOG_FILE

echo ""
echo "Serveur demarre en arriere-plan avec PID: $(cat server.pid)"
echo "Adresse: http://172.237.112.125:5000"
echo "Pour voir les logs: tail -f server-app.log"
echo "Pour verifier: ./check-server.sh"

...
useEffect(() => {
  ...
  const canSeeRejectedStatus = () => {
    ...
    return ['conformité', 'superviseur'].includes(currentUserRole);
  };
...
/*const loadDossiers = async () => {
  try {
    ...
    if (data.success) {
 	...
 	entreprise: d.sousDossiers[0]?.entreprise || 'Pas défini'
      }));
...
const loadDossiers = async () => {
  ...
  alert('Une erreur est survenue lors du chargement des données');
    ...
    if (data.success) {
      ...
      // DEBUG: Vérifier la structure des transferts
      const transfertsBruts = data.dossiers.filter((d: DossierClient) => 
        d.typeDocument === 'transfert'
      );
      
      console.log('🔍 TRANSFERTS BRUTS:', transfertsBruts.length, 'trouvés');
      
      transfertsBruts.forEach((transfert: DossierClient, index: number) => {
        console.log(`📋 Transfert ${index + 1}:`, {
          reference: transfert.reference,
          nom: transfert.nom,
          entrepriseDocument: transfert.entreprise, // Au niveau document
          sousDossiers: transfert.sousDossiers?.map((sd, sdIndex) => ({
            index: sdIndex,
            nom: sd.nom,
            entrepriseSousDossier: sd.entreprise, // Au niveau sous-dossier
            hasEntreprise: !!sd.entreprise,
            entrepriseValue: sd.entreprise
          }))
        });
      });
      ...
      // Convertir transferts avec DEBUG
      const transfertsFormates: DossierTransfert[] = filteredTransfert.map((d: DossierClient) => {
        const premierSousDossier = d.sousDossiers[0];
        const entrepriseValue = premierSousDossier?.entreprise || d.entreprise || 'Pas défini';
        
        console.log(`🔄 Conversion transfert ${d.reference}:`, {
          reference: d.reference,
          entrepriseDocument: d.entreprise,
          entrepriseSousDossier: premierSousDossier?.entreprise,
          entrepriseFinale: entrepriseValue,
          sousDossiersCount: d.sousDossiers.length
        });
        ...
        return {
          ...
          dateDebut: d.dateDebut,
          dateFin: d.dateFin,
          fichiers: premierSousDossier?.fichiers || [],
          statut: d.statut,
          dateCreation: d.dateCreation,
          motifRejet: d.motifRejet,
      ...
      console.log('✅ TRANSFERTS FORMATÉS:', transfertsFormates);
  ...
  if (activeTab === 'documents') {
    ...
    if (searchTerm.trim()) {
      filtered = filtered.filter(dossier =>
        ...
        dossier.profession?.toLowerCase().includes(searchTerm.toLowerCase()) || 
      (Array.isArray(dossier.pays) 
        ...
        dossier.typeTransfert?.toLowerCase().includes(searchTerm.toLowerCase()) ||  
	...
	transfert.entreprise?.toLowerCase().includes(searchTerm.toLowerCase()) ||
...
//  if (newStatut === 'rejeté' && currentUserRole === 'conformité') {
    if (newStatut === 'rejeté' && (currentUserRole === 'conformité' || currentUserRole === 'superviseur')) {
...
/*
  ...
  if (activeTab === 'documents') {
    ...
    dataToExport = filteredTransferts.map(transfert => ({
      ...
      'Sexe': transfert.sexe,
      'Entreprise': transfert.entreprise || 'Pas défini',
...
const exportToExcel = () => {
  ...
  if (activeTab === 'documents') {
    dataToExport = filteredDossiers.map(dossier => ({
      ...
      'Sexe': dossier.sexe,
      'Type': dossier.typeDocument,
      'Pays': Array.isArray(dossier.pays) ? dossier.pays.join(', ') : dossier.pays || 'N/A',
      'Type Transfert': dossier.typeTransfert || 'N/A',
      'Motif': dossier.motif || 'N/A',
      'Statut': dossier.statut,
      'Date de création': formatDate(dossier.dateCreation),
      'Date début': formatDate(dossier.dateDebut),
      'Date fin': formatDate(dossier.dateFin),
      'Nombre de sous-dossiers': dossier.sousDossiers.length
    }));
    filename = `documents_voyages_${new Date().toISOString().split('T')[0]}.csv`;
  } else if (activeTab === 'transferts') {
    dataToExport = filteredTransferts.map(transfert => ({
      'Référence': transfert.reference,
      'Nom': transfert.nom,
    ...
    filename = `dossiers_transferts_${new Date().toISOString().split('T')[0]}.csv`;
  }

  // Amélioration du format CSV pour Excel
  const headers = Object.keys(dataToExport[0] || {});
  
  // Utiliser le séparateur point-virgule pour une meilleure compatibilité Excel
  const csvContent = [
    ...
    ...dataToExport.map(row => 
      headers.map(h => {
        ...
        // Formater les dates et échapper les caractères spéciaux
...
/*  
// Utiliser URL.createObjectURL pour une meilleure compatibilité
    ...
    <div className="min-h-screen bg-gray-50 p-2 sm:p-4">
      <div className="max-w-full mx-auto px-1 sm:px-2">
        <div className="mb-6 sm:mb-8">
          <div className="bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-800 rounded-xl sm:rounded-2xl shadow-xl p-4 sm:p-6 mb-4 sm:mb-6">
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
              <div className="w-full lg:w-auto">
                <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1">
                  DataCollectApp
                  <span className="block text-lg sm:text-xl font-normal text-blue-100 mt-1 sm:mt-2">
                    Tableau de bord administrateur
                  </span>
                </h1>
                <div className="flex items-center bg-white/20 backdrop-blur-sm rounded-full px-3 sm:px-4 py-1 sm:py-2 mt-2 sm:mt-3">
                  <svg className="w-4 h-4 sm:w-5 sm:h-5 text-white mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                  </svg>
                  <span className="text-white font-medium mr-2 text-sm sm:text-base">{currentUser}</span>
              ...
              <button onClick={handleLogout} className="px-4 sm:px-6 py-2 sm:py-3 bg-white text-red-600 rounded-lg sm:rounded-xl hover:bg-red-50 hover:shadow-xl transition-all duration-300 transform hover:scale-105 flex items-center font-semibold group w-full lg:w-auto justify-center">
                <svg className="w-4 h-4 sm:w-5 sm:h-5 mr-2 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {activeTab === 'documents' && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4">
              <div className="bg-white rounded-lg p-4 sm:p-6 shadow-sm border hover:shadow-lg hover:scale-105 transition-all duration-300">
              ...
              <div className="bg-white rounded-lg p-4 sm:p-6 shadow-sm border hover:shadow-lg hover:scale-105 transition-all duration-300">
                <div className="flex items-center">
                  ...
                  <div className="ml-3 sm:ml-4">
                    <p className="text-xl sm:text-2xl font-bold text-gray-900">{stats.enAttente}</p>
              ...
              <div className="bg-white rounded-lg p-4 sm:p-6 shadow-sm border hover:shadow-lg hover:scale-105 transition-all duration-300">
                <div className="flex items-center">
                  ...
                  <div className="ml-3 sm:ml-4">
                    <p className="text-xl sm:text-2xl font-bold text-gray-900">{stats.traites}</p>
                    <p className="text-xs sm:text-sm text-gray-600">Apurés</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg p-4 sm:p-6 shadow-sm border hover:shadow-lg hover:scale-105 transition-all duration-300">
                <div className="flex items-center">
                  ...
                  <div className="ml-3 sm:ml-4">
                    <p className="text-xl sm:text-2xl font-bold text-gray-900">{stats.archives}</p>
                    <p className="text-xs sm:text-sm text-gray-600">Archives</p>
                  </div>
                </div>
              </div>
              ...
              {canSeeRejectedStatus() && (
                <div className="bg-white rounded-lg p-4 sm:p-6 shadow-sm border hover:shadow-lg hover:scale-105 transition-all duration-300">
                  <div className="flex items-center">
                    ...
                    <div className="ml-3 sm:ml-4">
                      <p className="text-xl sm:text-2xl font-bold text-gray-900">{stats.rejetes}</p>
                      <p className="text-xs sm:text-sm text-gray-600">Rejetés</p>
                    </div>
                  </div>
                </div>
              )}
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4">
    <div className="bg-white rounded-lg p-4 sm:p-6 shadow-sm border hover:shadow-lg hover:scale-105 transition-all duration-300">
      <div className="flex items-center">
        ...
        <div className="ml-3 sm:ml-4">
          <p className="text-xl sm:text-2xl font-bold text-gray-900">{stats.total}</p>
          <p className="text-xs sm:text-sm text-gray-600">Total transferts</p>
        </div>
      </div>
    </div>
    ...
    {canSeeRejectedStatus() && (
      <div className="bg-white rounded-lg p-4 sm:p-6 shadow-sm border hover:shadow-lg hover:scale-105 transition-all duration-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              ...
              <div className="bg-white rounded-lg p-4 sm:p-6 shadow-sm border hover:shadow-lg hover:scale-105 transition-all duration-300">
                <div className="flex items-center">
                  ...
                  <div className="ml-3 sm:ml-4">
                    <p className="text-xl sm:text-2xl font-bold text-gray-900">{stats.actifs}</p>
                ...
                <>
                  <div className="bg-white rounded-lg p-4 sm:p-6 shadow-sm border hover:shadow-lg hover:scale-105 transition-all duration-300">
                    <div className="flex items-center">
                      ...
                      <div className="ml-3 sm:ml-4">
                        ...
                        <p className="text-xs sm:text-sm text-gray-600">Admin Banque</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-lg p-4 sm:p-6 shadow-sm border hover:shadow-lg hover:scale-105 transition-all duration-300">
                    <div className="flex items-center">
                      ...
                      <div className="ml-3 sm:ml-4">
                        <p className="text-xl sm:text-2xl font-bold text-gray-900">{stats.superAdmins}</p>
                ...
                <>
                  <div className="bg-white rounded-lg p-4 sm:p-6 shadow-sm border hover:shadow-lg hover:scale-105 transition-all duration-300">
                    <div className="flex items-center">
                      ...
                      <div className="ml-3 sm:ml-4">
                        <p className="text-xl sm:text-2xl font-bold text-gray-900">{stats.superviseurs}</p>
                        <p className="text-xs sm:text-sm text-gray-600">Superviseurs</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-lg p-4 sm:p-6 shadow-sm border hover:shadow-lg hover:scale-105 transition-all duration-300">
                    <div className="flex items-center">
                      ...
                      <div className="ml-3 sm:ml-4">
                        <p className="text-xl sm:text-2xl font-bold text-gray-900">{stats.auditeurs}</p>
                        <p className="text-xs sm:text-sm text-gray-600">Auditeurs</p>
                      </div>
                    </div>
                  </div>
                </>
        ...
        <div className="bg-white rounded-lg p-4 sm:p-6 mb-6 sm:mb-8 shadow-sm border hover:shadow-lg transition-shadow duration-300">
...
/>
    ...
    />
            ...
            <div className="flex space-x-2">
              <button
                ...
                className="px-4 sm:px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 hover:shadow-lg transition-all duration-300 flex items-center"
              >
              ...
              <button
                onClick={resetSearch}
                className="px-3 sm:px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 hover:shadow-lg transition-all duration-300"
              >
                Reset
              </button>
            </div>
        ...
        <div className="flex space-x-1 mb-6 overflow-x-auto pb-2">
          {canViewDocuments() && (
            <button
              ...
              className={`px-4 sm:px-6 py-2 sm:py-3 rounded-lg transition-all duration-300 flex items-center flex-shrink-0 ${
                activeTab === 'documents'
          ...
          <div className="ml-auto flex space-x-2 flex-wrap justify-end gap-2">
            {(currentUserRole === 'auditeur' || currentUserRole === 'conformité') && (activeTab === 'documents' || activeTab === 'transferts') && (
              <button
                onClick={exportToExcel}
                className="px-4 sm:px-6 py-2 sm:py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 hover:shadow-lg transition-all duration-300 flex items-center flex-shrink-0"
              >
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {activeTab === 'documents' ? (
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl border-2 sm:border-4 border-blue-100 overflow-hidden">
            <div className="px-4 sm:px-6 py-4 sm:py-5 border-b-2 sm:border-b-4 border-blue-200 bg-gradient-to-r from-blue-600 to-indigo-600">
  ...
  <tr>
    <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Référence</th>
    <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Client</th>
    <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase hidden sm:table-cell">E-mail / Téléphone</th>
  ...
  {filteredDossiers.map((dossier) => (
    <tr key={dossier.reference} className="hover:bg-gradient-to-r hover:from-blue-50 hover:to-white transition-all duration-300">
      ...
      <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
        <div className="flex items-center">
          <div className={`w-8 h-8 sm:w-10 sm:h-10 ${dossier.typeDocument === 'voyage' ? 'bg-blue-100' : 'bg-purple-100'} rounded-full flex items-center justify-center mr-2 sm:mr-3`}>
            <svg className={`w-4 h-4 sm:w-5 sm:h-5 ${dossier.typeDocument === 'voyage' ? 'text-blue-600' : 'text-purple-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div>
            <p className="text-sm font-bold text-gray-900 truncate max-w-[120px] sm:max-w-none">{dossier.reference}</p>
      ...
      <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
        <div>
          <p className="text-sm font-medium text-gray-900 truncate max-w-[100px] sm:max-w-none">{dossier.nom} {dossier.prenom}</p>
          <p className="text-xs text-gray-500 truncate max-w-[100px] sm:max-w-none">{dossier.profession}</p>
      ...
      <td className="px-4 sm:px-6 py-4 whitespace-nowrap hidden sm:table-cell">
        <div className="flex flex-col space-y-1">
          ...
          >
            ...
            <span className="truncate max-w-[120px]">{dossier.email}</span>
          </button>
          {dossier.telephone && (
            <button
...
>
  ...
  <span className="truncate max-w-[100px]">{dossier.telephone}</span>
</button>
          )}
        </div>
      </td>
      ...
      <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
        <select
         ...
         disabled={!canModifyStatus()}
          className={`text-xs px-2 sm:px-3 py-1 rounded-full border-0 font-medium focus:ring-2 transition-all cursor-pointer ${
      ...
      <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
        <div className="flex justify-end space-x-1 sm:space-x-2 flex-wrap gap-1">
          <button
            onClick={() => openDossierModal(dossier)}
            className="text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 px-2 sm:px-3 py-1 rounded-lg transition-all duration-300 text-xs sm:text-sm"
          >
            Voir
          </button>
          {(currentUserRole === 'admin_bank' || currentUserRole === 'superviseur') && dossier.statut !== 'rejeté' && (
            <button
              ...
              className="text-red-600 hover:text-red-800 bg-red-50 hover:bg-red-100 px-2 sm:px-3 py-1 rounded-lg transition-all duration-300 text-xs sm:text-sm"
            >
              Supprimer
            </button>
          )}
  ...
  <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl border-2 sm:border-4 border-blue-100 overflow-hidden">
    <div className="px-4 sm:px-6 py-4 sm:py-5 border-b-2 sm:border-b-4 border-blue-200 bg-gradient-to-r from-blue-600 to-blue-700">
      <h3 className="text-lg sm:text-xl font-bold text-white flex items-center">
        <svg className="w-5 h-5 sm:w-6 sm:h-6 mr-2 sm:mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
        </svg>
        ...
        <thead className="bg-gradient-to-r from-blue-50 to-blue-100">
          <tr>
            ...
            <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase hidden sm:table-cell">Contact</th>
              <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Entreprise</th>
	    <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Type Transfert</th>
           <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Date dépôt</th>
            <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Statut</th>
            <th className="px-4 sm:px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Actions</th>
          </tr>
        </thead>
            ...
            <tr key={transfert.reference} className="hover:bg-gradient-to-r hover:from-blue-50 hover:to-white transition-all duration-300">
              ...
              <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="text-sm font-bold text-blue-700 truncate max-w-[100px] sm:max-w-none">{transfert.reference}</p>
...
<tbody className="bg-white divide-y divide-gray-200">
  {filteredTransferts.map((transfert) => {
    console.log(` Affichage transfert ${transfert.reference}:`, {
      entreprise: transfert.entreprise,
      nom: transfert.nom
    });
        ...
        <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
          <div className="flex items-center">
            ...
            <div>
              ...
              <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                <div>
                  <p className="text-sm font-medium text-gray-900 truncate max-w-[100px] sm:max-w-none">{transfert.nom} {transfert.prenom}</p>
                  <p className="text-xs text-gray-500 truncate max-w-[100px] sm:max-w-none">{transfert.profession}</p>
              ...
              <td className="px-4 sm:px-6 py-4 whitespace-nowrap hidden sm:table-cell">
                <div className="flex flex-col space-y-1">
                  ...
                  >
                    ...
                    <span className="truncate max-w-[120px]">{transfert.email}</span>
                  </button>
                  {transfert.telephone && (
                    <button
...
>
  ...
  <span className="truncate max-w-[100px]">{transfert.telephone}</span>
</button>
                  )}
                </div>
              </td>
...
<td className="px-4 sm:px-6 py-4">
  <p className="text-sm text-gray-900 truncate max-w-[120px]" title={transfert.entreprise || 'Pas défini'}>
    {transfert.entreprise || 'Pas défini'}
  </p>
</td>

              {/* Colonne Type Transfert*/} 
	<td className="px-4 sm:px-6 py-4">
  <p className="text-sm text-gray-900 truncate max-w-[150px]" title={transfert.typeTransfert}>
    {transfert.typeTransfert}
              ...
              <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div className="flex justify-end space-x-1 sm:space-x-2 flex-wrap gap-1">
                  <button
                    ...
                    className="text-blue-600 hover:text-purple-800 bg-blue-50 hover:bg-blue-100 px-2 sm:px-3 py-1 rounded-lg transition-all duration-300 text-xs sm:text-sm"
                  >
                    Voir
                  </button>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl border-2 sm:border-4 border-green-100 overflow-hidden">
               	<div className="px-4 sm:px-6 py-4 sm:py-5 border-b-2 sm:border-b-4 border-blue-200 bg-gradient-to-r from-blue-600 to-cyan-600">
                <h3 className="text-lg sm:text-xl font-bold text-white flex items-center">
                  <svg className="w-5 h-5 sm:w-6 sm:h-6 mr-2 sm:mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z" />
                  </svg>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Statut</th>
                      <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase hidden md:table-cell">Date création</th>
                      <th className="px-4 sm:px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                      ...
                      <tr key={user._id} className="hover:bg-gradient-to-r hover:from-green-50 hover:to-white transition-all duration-300">
                        <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            ...
                            <div>
                              <p className="text-sm font-medium text-gray-900 truncate max-w-[100px] sm:max-w-none">{user.username}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                          <span className="text-sm text-gray-900 truncate max-w-[120px] sm:max-w-none block">{user.email}</span>
                        </td>
                        <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                            ...
                            {user.role === 'admin_bank' ? 'Admin Banque' :
                             user.role === 'agent_saisie' ? 'Agent Saisie' :
                            ...
                            {(currentUserRole === 'admin_bank' || currentUserRole === 'super_admin') && (
                              <>
                                <button
                                  ...
                                  className="text-green-600 hover:text-green-800 bg-green-50 hover:bg-green-100 px-2 sm:px-3 py-1 rounded-lg transition-all duration-300 text-xs sm:text-sm"
                                >
                                  Modifier
                                </button>
                                <button
...
{showDossierModal && selectedDossier && (
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4 backdrop-blur-sm">
    <div className="bg-white rounded-xl sm:rounded-3xl max-w-full lg:max-w-6xl w-full max-h-[90vh] overflow-hidden shadow-2xl border-2 sm:border-4 border-blue-200 mx-2 sm:mx-4">
      
      {/* En-tête de la modal */}
      <div className="flex items-center justify-between p-4 sm:p-6 border-b-2 sm:border-b-4 border-blue-300 bg-gradient-to-r from-blue-600 to-indigo-600">
        <div className="flex-1 min-w-0">
          <h3 className="text-lg sm:text-xl font-semibold text-white truncate">
            {selectedDossier.typeDocument === 'voyage' ? 'Dossier Voyage' : 'Dossier'} : {selectedDossier.nom} {selectedDossier.prenom}
          </h3>
          <p className="text-blue-100 text-xs sm:text-sm mt-1 truncate">
            {selectedDossier.email} | {selectedDossier.telephone} | Référence: {selectedDossier.reference}
          </p>
        </div>
        <button
          onClick={() => setShowDossierModal(false)}
          className="text-white hover:text-gray-200 p-2 hover:bg-white/20 rounded-full transition-all duration-300 flex-shrink-0 ml-2"
        >
      ...
      <div className="p-4 sm:p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
          ...
          <div className="bg-blue-50 p-3 sm:p-4 rounded-lg">
            <h4 className="font-medium text-gray-900 mb-2 sm:mb-3 flex items-center">
            ...
            <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm">
              <div>
                ...
                <p className="text-gray-900">{selectedDossier.nom} {selectedDossier.prenom}</p>
              </div>
              <div>
                <span className="font-medium text-gray-500">Email:</span>
                <p className="text-gray-900">{selectedDossier.email}</p>
              </div>
              <div>
                <span className="font-medium text-gray-500">Téléphone:</span>
                <p className="text-gray-900">{selectedDossier.telephone || 'Non renseigné'}</p>
              </div>
              <div>
                <span className="font-medium text-gray-500">Profession:</span>
                <p className="text-gray-900">{selectedDossier.profession}</p>
              </div>
              <div>
                <span className="font-medium text-gray-500">Sexe:</span>
                <p className="text-gray-900">{selectedDossier.sexe === 'H' ? 'Homme' : 'Femme'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-indigo-50 p-3 sm:p-4 rounded-lg">
            ...
            <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm">
              <div>
                ...
                <p className="text-gray-900">{selectedDossier.typeDocument === 'voyage' ? 'Voyage' : 'Transfert'}</p>
              </div>
              <div>
                <span className="font-medium text-gray-500">Référence:</span>
                <p className="text-gray-900">{selectedDossier.reference}</p>
              </div>
              <div>
              ...
              <div>
                <span className="font-medium text-gray-500">Date création:</span>
                <p className="text-gray-900">{formatDate(selectedDossier.dateCreation)}</p>
              </div>
              <div>
                <span className="font-medium text-gray-500">Sous-dossiers:</span>
                <p className="text-gray-900">{selectedDossier.sousDossiers.length}</p>
              </div>
            </div>
          </div>
        </div>

        {/* SECTION DES SOUS-DOSSIERS - GARDÉE IDENTIQUE */}
        <div className="bg-white border border-blue-200 rounded-xl p-4 sm:p-6">
          ...
          <div className="h-full max-h-full overflow-y-auto border border-gray-200 rounded-lg bg-gray-50 p-3 sm:p-4">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
              {selectedDossier.sousDossiers.map((sousDossier, index) => (
                <div key={index} className="border border-gray-200 rounded-xl p-3 sm:p-4 hover:shadow-md hover:border-blue-300 transition-all duration-300 bg-white">
                  <div className="flex flex-col h-full">
                    ...
                    <div className="flex items-start justify-between mb-2 sm:mb-3">
                      <div className="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          <h5 className="font-medium text-gray-900 truncate text-sm sm:text-base" title={sousDossier.nom}>
                            {sousDossier.nom}
                          </h5>
                          <p className="text-xs text-gray-600">{sousDossier.fichiers.length} fichier(s)</p>
                          ...
                          {selectedDossier.typeDocument === 'voyage' ? (
                            <div className="mt-1 sm:mt-2 text-xs text-gray-600">                 
                              <p className="truncate"><strong>Pays:</strong> {Array.isArray(sousDossier.pays) ? sousDossier.pays?.join(', ') : sousDossier.pays}</p>
                              <p className="truncate"><strong>Raison:</strong> {sousDossier.raison === 'autres' ? sousDossier.autreRaison : sousDossier.raison}</p>
                            </div>
                          ) : (
                            <div className="mt-1 sm:mt-2 text-xs text-gray-600">
                              <p className="truncate"><strong>Type:</strong> {sousDossier.typeTransfert}</p>
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-between mt-auto pt-2 sm:pt-3 border-t border-gray-100">
                      <button
                        ...
                        className="text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg transition-all duration-300 text-xs sm:text-sm font-medium flex items-center"
                      >
                      ...
                      {canDeleteSubfoldersAndFiles() && selectedDossier.statut !== 'rejeté' && (
                        <button
                          ...
                          className="text-red-600 hover:text-red-800 bg-red-50 hover:bg-red-100 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg transition-all duration-300 text-xs sm:text-sm font-medium flex items-center"
                        >
                          <svg className="w-3 h-3 sm:w-4 sm:h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {selectedDossier.sousDossiers.length === 0 && (
            <div className="text-center py-6 sm:py-8 text-gray-500">
              ...
              <p className="mt-2 text-sm">Aucun sous-dossier dans ce dossier</p>
            </div>
          )}
        </div>

        {/* Actions */}
        <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t flex justify-end space-x-2 sm:space-x-3">
          <button
            onClick={() => setShowDossierModal(false)}
            className="px-3 sm:px-4 py-2 text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-all duration-300 text-sm"
          >
            Fermer
          </button>
          {(currentUserRole === 'admin_bank' || currentUserRole === 'superviseur') && selectedDossier.statut !== 'rejeté' && (
            <button
              onClick={() => confirmDelete('dossier', selectedDossier)}
              className="px-3 sm:px-4 py-2 text-white bg-red-600 hover:bg-red-700 rounded-lg transition-all duration-300 text-sm"
            >
...
{showSousDossierModal && selectedSousDossier && selectedDossier && (
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4 backdrop-blur-sm">
    <div className="bg-white rounded-xl sm:rounded-3xl max-w-full sm:max-w-3xl w-full max-h-[90vh] overflow-hidden shadow-2xl border-2 sm:border-4 border-blue-200 mx-2 sm:mx-4">
      <div className="flex items-center justify-between p-4 sm:p-6 border-b-2 sm:border-b-4 border-blue-300 bg-gradient-to-r from-blue-600 to-indigo-600">
        <div className="flex-1 min-w-0">
          <h3 className="text-lg sm:text-xl font-semibold text-white truncate">
            {selectedDossier.typeDocument === 'voyage' ? 'Sous-dossier Voyage' : 'Sous-dossier Transfert'}: {selectedSousDossier.nom}
          </h3>
          <p className="text-blue-100 text-xs sm:text-sm mt-1">
            Créé le {formatDate(selectedSousDossier.date)} | {selectedSousDossier.fichiers.length} fichier(s)
          </p>
        </div>
        <button
      ...
      <div className="p-4 sm:p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
        ...
        <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-blue-50 rounded-lg">
          <h4 className="font-medium text-gray-900 mb-2 sm:mb-3">Détails du {selectedDossier?.typeDocument === 'voyage' ? 'voyage' : 'transfert'}</h4>
       ...
       <div><span className="font-medium">Période:</span> Du {selectedSousDossier?.dateDebut ? new Date(selectedSousDossier.dateDebut).toLocaleDateString('fr-FR') : 'N/A'} au {selectedSousDossier?.dateFin ? new Date(selectedSousDossier.dateFin).toLocaleDateString('fr-FR') : 'N/A'}</div>
        ...
        <div className="space-y-2 sm:space-y-3">
          {selectedSousDossier.fichiers.map((fichier, index) => (
            <div key={index} className="flex items-center justify-between p-3 sm:p-5 border border-blue-200 rounded-xl bg-gradient-to-r from-blue-50 to-white">
              <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-gray-600">{formatFileSize(fichier.taille)} • {fichier.extension}</p>
                  <p className="text-xs text-gray-500 truncate">Fichier système: {fichier.nom}</p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-1 sm:space-x-2 flex-shrink-0 ml-2">
                <button
                  ...
                  className="text-green-600 hover:text-green-800 bg-green-50 hover:bg-green-100 px-2 sm:px-3 py-1 rounded transition-all duration-300 text-xs sm:text-sm"
                >
                  Télécharger
                </button>
                {canDeleteSubfoldersAndFiles() && selectedDossier?.statut !== 'rejeté' && (
                  <button
                    ...
                    className="text-red-600 hover:text-red-800 bg-red-50 hover:bg-red-100 px-2 sm:px-3 py-1 rounded transition-all duration-300 text-xs sm:text-sm"
                  >
                    Supprimer
                  </button>
                )}
        ...
        <div className="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t flex justify-end space-x-2 sm:space-x-3">
          <button
            onClick={() => setShowSousDossierModal(false)}
            className="px-3 sm:px-4 py-2 text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-all duration-300 text-sm"
          >
  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4 backdrop-blur-sm">
    <div className="bg-white rounded-xl sm:rounded-3xl max-w-full lg:max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl border-2 sm:border-4 border-blue-200 mx-2 sm:mx-4">
      <div className="flex items-center justify-between p-4 sm:p-6 border-b-2 sm:border-b-4 border-purple-300 bg-gradient-to-r from-blue-600 to-blue-700">
        <div className="flex-1 min-w-0">
          <h3 className="text-lg sm:text-xl font-semibold text-white truncate">Dossier Transfert: {selectedTransfert.reference}</h3>
          <p className="text-purple-100 text-xs sm:text-sm mt-1">Créé le {formatDate(selectedTransfert.dateCreation)}</p>
        </div>
        <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
          <div className="bg-blue-50 p-3 sm:p-4 rounded-lg">
            ...
            <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm">
              <div>
                <span className="font-medium text-gray-500">Nom complet:</span>
                <p className="text-gray-900">{selectedTransfert.nom} {selectedTransfert.prenom}</p>
              </div>
              <div>
                <span className="font-medium text-gray-500">Email:</span>
                <p className="text-gray-900">{selectedTransfert.email}</p>
              </div>
              <div>
                <span className="font-medium text-gray-500">Téléphone:</span>
                <p className="text-gray-900">{selectedTransfert.telephone || 'Non renseigné'}</p>
              </div>
              <div>
                <span className="font-medium text-gray-500">Profession:</span>
                <p className="text-gray-900">{selectedTransfert.profession}</p>
              </div>
              <div>
            <div>
  <span className="font-medium text-gray-500">Entreprise:</span>
  <p className="text-gray-900">{selectedTransfert.entreprise || 'Pas défini'}</p>
</div>
                <span className="font-medium text-gray-500">Sexe:</span>
                <p className="text-gray-900">{selectedTransfert.sexe === 'H' ? 'Homme' : 'Femme'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-purple-50 p-3 sm:p-4 rounded-lg">
            ...
            <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm">
              <div>
                <span className="font-medium text-gray-500">Type de transfert:</span>
                <p className="text-gray-900">{selectedTransfert.typeTransfert}</p>
              </div>
            
<div>
  <span className="font-medium text-gray-500">Date de dépôt:</span>
  <p className="text-gray-900">
    {formatDate(selectedTransfert.dateCreation)}
  </p>
</div>
              <div>
                <span className="font-medium text-gray-500">Statut:</span>
              ...
              {selectedTransfert.motifRejet && (
                <div>
                  <span className="font-medium text-gray-500">Motif rejet:</span>
                  <p className="text-red-600 text-sm">{selectedTransfert.motifRejet}</p>
                </div>
              )}
            </div>
          </div>
          ...
          {selectedTransfert.fichiers.length > 0 ? (
            <div className="space-y-2 sm:space-y-3">
              {selectedTransfert.fichiers.map((fichier, index) => (
                <div key={index} className="flex items-center justify-between p-3 sm:p-4 border border-blue-100 rounded-lg bg-purple-50/50">
                  <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
  ...
  <button
    ...
    className="text-blue-600 hover:text-purple-800 bg-blue-100 hover:bg-purple-200 px-2 sm:px-3 py-1 rounded transition-all duration-300 text-xs sm:text-sm"
  >
    Télécharger
  </button>
    ...
    >
      Supprimer
            ...
            <div className="text-center py-6 sm:py-8 text-gray-500">
              ...
              <p className="mt-2 text-sm">Aucun fichier dans ce dossier</p>
            </div>
          )}
        </div>
        ...
        {showCreateUserModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4 backdrop-blur-sm">
            <div className="bg-white rounded-lg max-w-full sm:max-w-md w-full shadow-2xl mx-2 sm:mx-4">
              <div className="flex items-center justify-between p-4 sm:p-6 border-b bg-gradient-to-r from-blue-50 to-white">
                <h3 className="text-lg sm:text-xl font-semibold text-gray-900">Créer un utilisateur</h3>
                <button
              ...
              <div className="p-4 sm:p-6">
                ...
                <div className="mt-4 sm:mt-6 flex justify-end space-x-2 sm:space-x-3">
                  ...
                  <button
                    onClick={createUser}
                    className="px-3 sm:px-4 py-2 text-white bg-green-600 hover:bg-green-700 rounded-lg transition-all duration-300 text-sm"
                  >
                    Créer
                  </button>
                </div>
        ...
        {showUserModal && selectedUser && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4 backdrop-blur-sm">
            <div className="bg-white rounded-lg max-w-full sm:max-w-md w-full shadow-2xl mx-2 sm:mx-4">
              <div className="flex items-center justify-between p-4 sm:p-6 border-b bg-gradient-to-r from-blue-50 to-white">
                <h3 className="text-lg sm:text-xl font-semibold text-gray-900">Modifier l'utilisateur</h3>
                <button
         ...
         {showRejectModal && (dossierToReject || transfertToReject) && (
          <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-2 sm:p-4 backdrop-blur-md">
            <div className="bg-white rounded-xl sm:rounded-2xl max-w-full sm:max-w-lg w-full shadow-2xl border-2 sm:border-4 border-red-200 mx-2 sm:mx-4">
              <div className="p-4 sm:p-6 border-b-2 sm:border-b-4 border-red-300 bg-gradient-to-r from-red-600 to-rose-600">
                <div className="flex items-center">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 bg-white/20 backdrop-blur rounded-full flex items-center justify-center mr-3 sm:mr-4">
                    <svg className="w-5 h-5 sm:w-7 sm:h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-xl sm:text-2xl font-bold text-white truncate">Motif de rejet</h3>
                    <p className="text-red-100 text-xs sm:text-sm mt-1 truncate">Dossier: {dossierToReject?.nom || transfertToReject?.nom}</p>
                  </div>
                </div>
              </div>

              <div className="p-4 sm:p-6 bg-gradient-to-br from-red-50 to-white">
                <label className="block text-sm font-semibold text-gray-800 mb-2 sm:mb-3 flex items-center">
                ...
                <textarea
                  ...
                  className="w-full px-3 sm:px-4 py-2 sm:py-3 border border-red-200 rounded-lg sm:rounded-xl focus:ring-2 focus:ring-red-500/30 focus:border-red-500 min-h-[120px] sm:min-h-[140px] transition-all duration-300 hover:border-red-300 bg-white/80 backdrop-blur text-sm"
                  placeholder="Expliquez en détail pourquoi ce dossier est rejeté (documents manquants, non conformes, etc.)..."
                  required
                />
                <p className="text-xs text-gray-600 mt-1 sm:mt-2 flex items-center">
              ...
              <div className="p-4 sm:p-6 border-t border-red-100 bg-gray-50 flex justify-end space-x-2 sm:space-x-3">
                <button
                  onClick={() => {
                    setShowRejectModal(false);
                    setRejectReason('');
                  ...
                  className="px-4 sm:px-6 py-2 bg-white text-gray-700 rounded-lg sm:rounded-xl hover:bg-gray-100 border border-gray-300 hover:border-gray-400 transition-all duration-300 font-medium text-sm"
                >
                  Annuler
                </button>
                <button
                  ...
                  className={`px-4 sm:px-6 py-2 rounded-lg sm:rounded-xl font-medium transition-all duration-300 flex items-center text-sm ${
                    rejectReason.trim()
              ...
              <div className="p-4 sm:p-6">
                ...
                <div className="flex space-x-2 sm:space-x-4">
                  <button
                    ...
                    className="flex-1 px-3 sm:px-4 py-2 text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-all duration-300 text-sm"
                  >
                    Annuler
                  </button>
                  <button
                    onClick={handleDelete}
                    className="flex-1 px-3 sm:px-4 py-2 text-white bg-red-600 hover:bg-red-700 rounded-lg transition-all duration-300 text-sm"
                  >
                    Supprimer
                  </button>
                </div>
